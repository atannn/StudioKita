<x-owner-layout title="Edit Jadwal">
    <div class="max-w-3xl mx-auto space-y-4">
        ...
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900 dark:text-gray-100">
                <form method="POST" action="{{ route('owner.jadwals.update', $jadwal->idJadwal) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-5">
                            ...
                            <select id="service_idservice"
                                ...
                                @foreach ($services as $service)
                                    <option value="{{ $service->idservice }}"
                                            ...
                                            @selected(old('service_idservice', $jadwal->service_idservice) == $service->idservice)>
                            ...
                            <select id="rooms_idrooms"
                                ...
                                @foreach ($rooms as $room)
                                    <option value="{{ $room->idrooms }}"
                                            ...
                                            @selected(old('rooms_idrooms', $jadwal->rooms_idrooms) == $room->idrooms)>
                        ...
                        <div>
                            ...
                            <select name="status"
                                    class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 focus:border-indigo-500 focus:ring-indigo-500"
                                    required>
                                <option value="available" @selected(old('status', $jadwal->status) === 'available')>Tersedia</option>
                                <option value="booked" @selected(old('status', $jadwal->status) === 'booked')>Dibooking</option>
                                <option value="blocked" @selected(old('status', $jadwal->status) === 'blocked')>Diblokir</option>
                            </select>
                        </div>
